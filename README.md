...
Gold    : gold_sector_war_summary, gold_stock_ranking,
          gold_weekly_sector_performance, gold_war_daily_timeline
          fact_daily_prices, dim_date, dim_company, dim_sector [Star Schema]
```
...
### Monitoring
```
Airflow Task Failure → on_failure_callback → Discord Pipeline Alert
Realtime Price Anomaly → Consumer → Discord Price Alert
```

**Stack:** Apache Airflow 2.7.1 · MongoDB · Apache Kafka · Streamlit · Docker Compose · Python 3.8
...
- ✅ **Star Schema** — fact_daily_prices + 3 dimensions (date, company, sector)
- ✅ **Incremental Processing** — Watermark pattern + Bulk Upsert (idempotent)
- ✅ **Data Quality Gate** — 5 rules ครอบคลุม Silver และ Gold ทั้งหมด 11 จุด
- ✅ **Real-time Streaming** — Continuous Kafka consumer 
- ✅ **Smart Symbol Selection** — เลือก 50 หุ้นจากผล Batch Analysis
- ✅ **Discord Alert System** — Price alerts (6 ประเภท) + Pipeline failure alerts
- ✅ **Multi-source Live Price** — Finnhub → yfinance intraday → silver close → yfinance daily
...
|---|---|---|
| `sp500_reference_data_pipeline` | weekly | อัปเดตรายชื่อหุ้น S&P 500 |
| `sp500_company_profiles_pipeline` | monthly | Smart Refresh (skip ถ้า < 30 วัน) |
| `sp500_daily_analytics_pipeline` | `0 18 * * 1-5` (จ-ศ 18:00 UTC) | Build Silver + Gold + Star Schema |
...
| `live_trades` | streaming | Real-time prices จาก Finnhub |
...
| `silver_war_impact_analysis` | 11 | sector-level analysis |
| `silver_company_enriched` | 503 | + war_impact + sector fallback |
| `silver_live_trades` | streaming | + sector + war_impact |

### Gold Layer — Data Marts (สำหรับ Dashboard)
| Collection | คำอธิบาย |
|---|---|
| `gold_sector_war_summary` | สรุปรายกลุ่มอุตสาหกรรม |
| `gold_stock_ranking` | จัดอันดับหุ้น winners/losers |
| `gold_weekly_sector_performance` | performance รายสัปดาห์ |
| `gold_war_daily_timeline` | timeline รายวัน |

### Gold Layer — Star Schema (Kimball)
| Collection | จำนวน | บทบาท |
|---|---|---|
| `fact_daily_prices` | 127,129 | **Fact Table** (FK: date_sk, company_sk, sector_sk) |
| `dim_date` | 253 | มิติวันที่ + period |
| `dim_company` | 503 | มิติบริษัท + war_impact |
| `dim_sector` | 11 | มิติ sector + war_impact_label |
...
│   │
│   ├── silver/
│   │   └── silver_layer.py             # 5 transformations + DQ
│   │
│   ├── gold/
│   │   └── gold_layer.py               # 4 Data Marts + 4 Star Schema
│   │
│   ├── pipelines/
│   │   ├── sp500_daily_analytics_pipeline.py   # Main DAG (13 tasks)
...
│   └── utils/
│       ├── __init__.py
│       ├── data_quality.py             # 5-rule DQ checker
│       └── alert_notifier.py           # Discord webhook (Airflow side)
│
├── realtime/
│   ├── realtime_producer.py    # Finnhub → Kafka (Smart 50 symbols)
...
Producer เลือกหุ้นอัตโนมัติจากผล Batch Analysis:
...
- ตั้งค่า `on_failure_callback` ใน Airflow DAG
...
DQ Gate ทำงานก่อน insert ทุกครั้ง — ถ้า CRITICAL → raise exception → Airflow fail → Discord alert
...
```javascript
db.fact_daily_prices.aggregate([
  { $lookup: {
      from: "dim_company",
...
- Final Report (Chapter 4-6) — `report_ch4_6.docx`
